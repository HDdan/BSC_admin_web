<template>
  <div class="potential-distributor__search">
    ...
    <div
        class="potential-distributor__upload ml-20 mb-11"
        @click="dialogFileVisible = !dialogFileVisible" v-if="showImport"
      >
...
export default {
  ...
  methods: {
      downTemplate(){
      window.open('http://dealer.qtdatas.com/file/template/OBOR-线上线下赋能主数据.xlsx')
    },
    fileDownLoad() {
      let list={
        filter:this.search,
  ...
  computed: {
    ...
    showImport() {
      return actionRoleServices.checkCourseInfo(this.userType);
    },
  }
};
</script>
